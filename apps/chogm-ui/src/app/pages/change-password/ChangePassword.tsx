...
export default function ChangePassword() {
	...
	const code = new URLSearchParams(search).get('verification_code');
	...
	const [resetPassword] = useMutation(RESET_PASSWORD, {
		onCompleted: (data) => {
			if (data?.resetPassword) {
				notifySuccess('You have successfully changed your password. Go ahead and login');
				routeChange('/');
			}
		},
	...
	const handleSubmit = (e: any) => {
		...
		resetPassword({ variables: { newPassword, verificationCode: code && parseFloat(code) } });
	}
	...
	return (
		<div className="flex items-center justify-center min-h-screen bg-sky-900">
			...
			<div className="z-20 w-full px-6 pt-12 pb-6 bg-white shadow-xl md:w-fit md:px-12 rounded-2xl">
				<div>
					<h1 className="mb-4 text-3xl font-bold text-center cursor-pointer">Change Password</h1>
				...
				<form onSubmit={handleSubmit}>
						...
						<div className="w-full md:w-[20rem]">
							<div className="relative">
								<input
									...
									className={`block w-full px-4 py-3 text-sm border rounded-lg outline-none ${invalidPwd && 'border-red-500 focus:ring-red-500 focus:border-red-500'}`}
									required
					...
					<div className="mt-6 text-center">
						<button className="flex items-center justify-center w-full py-3 text-xl text-white transition duration-150 ease-in-out bg-teal-500 rounded-xl disabled:bg-teal-700" type="submit" disabled={loading || invalidPwd}>
							<span className={`${loading && 'text-gray-300'}`}>Change</span>
							{loading && (	
								<span className="ml-2">
